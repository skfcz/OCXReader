...
shipxml::AMCurve ReadCurve(const LDOM_Element& curveRootN, const MajorPlane mp,
                           const gp_Dir normal) {
  auto meta = ocx::helper::GetOCXMeta(curveRootN);

  AMCurve crv = shipxml::AMCurve(ToAMSystem(normal));
  ...
  OCX_INFO("Read curve and split into #{} arcs", crv.GetSegments().size());


  return crv;
}
std::vector<ArcSegment> ReadNURBS3D(const LDOM_Element&  nurbs3DN, AMCurve amCurve) {
  ...
  OCX_DEBUG("u [{}, {}], delta u {}, #{}", uStart, uEnd, deltaU, deltas);
  ...
  OCX_DEBUG( "created #{} points", points.size());

//    for ( int i = 0; i < points.size();i++) {
//      OCX_DEBUG( "#{} {}", i, ToString( Convert(points.at(i))))
//    }
  ...
  while (currentEnd < points.size()) {
    //OCX_DEBUG("check from #{} to #{}", currentStart, currentEnd)
...
//              ToString(Convert(arcItm)));
    ...
    if (mkacc.IsDone()) {
      ...
      // OCX_DEBUG("created circle at {},r {}", ToString(Convert(center)), radius)
      ...
      for (int i = currentStart + 1; i < currentEnd; i++) {
        ...
        if (abs(center.Distance(pointToCheck) - radius) > tolerance) {
          ...
          OCX_DEBUG("check failed at {}", ToString(Convert(center)))
          isValid = false;
          break;
        }
      }
      ...
      if (mkln.IsDone()) {
        ...
        OCX_DEBUG("check as line")

        //   vec3 d = (C - B) / C.distance(B);
        gp_Vec d = gp_Vec(arcStart, arcEnd);
        d.Normalize();
        for (int i = currentStart + 1; i < currentEnd; i++) {
          gp_Pnt pointToCheck = points.at(i);

          // vec3 v = A - B;
          gp_Vec v = gp_Vec(arcStart, pointToCheck);
          // double t = v.dot(d);
          double t = v.Dot(d);
          // vec3 P = B + t * d;
          gp_Pnt P = gp_Pnt(arcEnd.X() + t * d.X(), arcEnd.Y() + t * d.Y(),
                            arcEnd.Y() + t * d.Y());
          if (P.Distance(pointToCheck) > tolerance) {
            // check failed
            OCX_DEBUG("check failed at {}", ToString(Convert(pointToCheck)))
            isValid = false;
            break;
          }
        ...
        OCX_ERROR("failed to create a line from {} to {}",
    ...
    if (isValid) {
      ...
      currentEnd++;
      // special handling for the last arc in case that lastValidEnd == last point
      //OCX_DEBUG("check passed at #{}, next check for #{}, {}",lastValidEnd, currentEnd, isArc ? " as arc" :  "as line")
    ...
    OCX_DEBUG("check failed, go back to #{} to #{}, create {}",currentStart, lastValidEnd, lastValidIsArc ? "arc": "line")
    gp_Pnt foundStart = points.at(currentStart);
    gp_Pnt end = points.at(lastValidEnd);

    //OCX_DEBUG("    start {}, end {}", ToString(Convert(foundStart)), ToString(Convert(end)))

    if (lastValidIsArc) {
      int iItm = (lastValidEnd - currentStart) < 3
                     ? currentStart + 1
                     : (int)ceil((currentStart + lastValidEnd) / 2.0);
      auto arcItm = points.at(iItm);

      //OCX_DEBUG("    found onArc #{}, {}", iItm,  ToString(Convert(arcItm)))

      GC_MakeArcOfCircle mkfacc(foundStart, end, arcItm);
      if ( ! mkfacc.IsDone()) {
        OCX_ERROR("    failed to create arc")
      }

      Handle(Geom_TrimmedCurve) arc = mkfacc.Value();
      Handle(Geom_Curve) baseCurve = arc->BasisCurve();
      Handle(Geom_Circle) circle = Handle(Geom_Circle)::DownCast(baseCurve);
      ...
      ArcSegment arcSegment =
          ArcSegment(Convert(arcStart), Convert(arcEnd), Convert(arcItm),
                     Convert(circle->Location()), true);
      segments.push_back(arcSegment);


    } else {
      segments.push_back(ArcSegment(Convert(foundStart), Convert(end)));
    }
  ...
  if ( lastValidEnd+1 < points.size()) {
    OCX_DEBUG("add closing line from #{} to #{}", currentStart, points.size()-1);
    auto arcStart = points.at(currentStart);
    auto arcEnd = points.at(points.size()-1);
    segments.push_back(ArcSegment(Convert(arcStart), Convert(arcEnd)));
  }

  OCX_DEBUG("created #{} arcs", segments.size());

  return segments;
}
std::vector<ArcSegment> ReadPolyLine3D(const LDOM_Element& crvEL, AMCurve curve) {
  return std::vector<ArcSegment>();
}
std::vector<ArcSegment> ReadLine3D(const LDOM_Element& lineEL, AMCurve curve) {
  ...
  segments.push_back(ArcSegment(Convert(start), Convert(end)));

  return segments;
}
std::vector<ArcSegment> ReadCircumArc3D(const LDOM_Element& circleN, AMCurve curve) {
  ...
  gp_Vec c2s = gp_Vec(center, start);
  gp_Vec c2e = gp_Vec(center, end);
  ...
  gp_Vec reference =
      curve.GetSystem() == YZ
          ? gp_Vec(1, 0, 0)
          : (curve.GetSystem() == XY ? gp_Vec(0, 0, 1) : gp_Vec(0, -1, 0));
  bool withershins = crossed.Angle(reference) > 0;

  // TODO: check if alpha > PI/2, then split into to ArcSegments

  ArcSegment arcSegment =
      ArcSegment(Convert(start), Convert(end), Convert(intermediate),
                 Convert(center), withershins);
  segments.push_back(arcSegment);

  return segments;
}

std::vector<ArcSegment> ReadCircle3D(const LDOM_Element& circleN, AMCurve curve) {
  ...
  ArcSegment arcSegment = ArcSegment(Convert(arcStart), Convert(arcEnd),
                                     Convert(arcIm1), Convert(center), true);
  segments.push_back(arcSegment);

  arcSegment = ArcSegment(Convert(arcEnd), Convert(arcStart), Convert(arcIm2),
                          Convert(center), true);
  segments.push_back(arcSegment);

  return segments;
}
std::vector<ArcSegment> ReadCircumCircle3D(const LDOM_Element& crvEL, AMCurve curve) {
  OCX_ERROR("ReadCircumCircle3D is not implemented")

  return std::vector<ArcSegment>();
}
std::vector<ArcSegment> ReadEllipse3D(const LDOM_Element& crvEL, AMCurve curve) {
  OCX_ERROR("ReadEllipse3D is not implemented")
  return std::vector<ArcSegment>();
}
std::vector<ArcSegment> ReadCompositeCurve3D(const LDOM_Element& crvEL,
                                             AMCurve curve) {
  OCX_ERROR("ReadCompositeCurve3D is not implemented")
