...
void shipxml::PanelReader::ReadPanels() const {
  LDOM_Node aChildNode = ocxVesselEL.getFirstChild();
...
shipxml::Panel shipxml::PanelReader::ReadPanel(LDOM_Element panelO,
  ...
  cout << "ReadPanel " << name << " (" << guid << ")"
       << panelO.getTagName().GetString() << "\n";
  ...
  for (int i = 0; i < attrs.getLength(); i++) {
    ...
    panel.Properties().Add(string(node.getNodeName().GetString()),
                           string(node.getNodeValue().GetString()));
  }
  ...
  // the support
  auto unboundedGeometryO =
      ShipXMLHelper::GetFirstChild(panelO, "UnboundedGeometry");
...
void shipxml::PanelReader::ReadLimits(LDOM_Element limitedByO,
                                      Panel panelX) const {
    ...
    if (aNodeType == LDOM_Node::ELEMENT_NODE) {
      ...
      if (ShipXMLHelper::GetLocalTagName(aElement) == "FreeEdgeCurve3D") {
        ...
        if (refType.find("GridRef") != std::string::npos) {
          ...
          cerr << "found unsupported refType in OcxItemPtr  " << localRef
               << " (" << guidRef << ") of Panel " << panelX.Name() << endl;
        }
        ...
        cerr << "found unsupported child element "
             << ShipXMLHelper::GetLocalTagName(aElement)
             << " in limitedBy of Panel " << panelX.Name() << endl;
      }
    }
    aChildNode = aChildNode.getNextSibling();
  }
...
shipxml::PanelReader::PanelReader(const LDOM_Element &vesselN,
                                  shipxml::ShipSteelTransfer *sstI,
                                  ocx::OCXContext *ctx) {
  ocxVesselEL = vesselN;
  sst = sstI;
  ocxContext = ctx;
