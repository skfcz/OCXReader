...
TopoDS_Wire OCXPanelReader::ReadPanelOuterContour(LDOM_Element &panelN) {
  ...
  if (outerContourN.isNull()) {
    std::cerr << "could not find OuterContour for Panel "
...
TopoDS_Shape OCXPanelReader::ReadPanelSurface(LDOM_Element &panelN,
  ...
  if (unboundedGeometryN.isNull()) {
    std::cerr << "could not find UnboundedGeometry for Panel " << id
              << std::endl;
    return shell;
  }

  // The unbounded geometry is either a shell, or a shell reference, or a grid
  // reference
  try {
    ...
    if (!gridRefN.isNull()) {
      std::string guid =
          std::string(gridRefN.getAttribute(ctx->OCXGUIDRef()).GetString());

      std::cout << "use GridRef " << guid << " as reference shell" << std::endl;
      shell = ctx->LookupSurface(guid);
      if (shell.IsNull()) {
        std::cerr << "failed to lookup grid reference to " << guid
                  << " contained in panel " << id << std::endl;
        return shell;
      }
    ...
    if (!surfaceRefN.isNull()) {
      ...
      std::cout << "use SurfaceRef " << guid << " as reference shell"
                << std::endl;
      shell = ctx->LookupSurface(guid);
      if (shell.IsNull()) {
        std::cerr << "failed to lookup shell reference to " << guid
                  << " contained in panel " << id << std::endl;
        return shell;
      }
...
TopoDS_Shape OCXPanelReader::ReadPlates(LDOM_Element &panelN,
...
TopoDS_Shape OCXPanelReader::ReadPlate(LDOM_Element &plateN,
  ...
  if (outerContourN.isNull()) {
    std::cerr << "could not find OuterContour for Plate " << id << " (" << guid
              << ")" << std::endl;
    return plateShape;
  }

  // material design green 50
  Quantity_Color plateColor =
  ...
  try {
    ...
    if (curveShape.ShapeType() != TopAbs_WIRE) {
      std::cerr << "expect a closed shape as OuterContour, but got "
                << curveShape.ShapeType() << " in plate " << id << " (" << guid
                << ")" << std::endl;
      return plateShape;
    }
    ...
    if (referenceSurface.ShapeType() == TopAbs_FACE) {
      ...
      if (restricted.IsNull()) {
        std::cerr
            << "failed to create a restricted surface Panel/UnboundedGeometry "
               "+ Plate/OuterContour contained in panel "
            << id << ", plate " << id << " (" << guid
            << ") : " << faceBuilder.Error() << std::endl;
        return plateShape;
      }
...
TopoDS_Shape OCXPanelReader::ReadStiffener(LDOM_Element &stiffenerN) {
  ...
  try {
    ...
    if (traceN.isNull()) {
      ...
      if (trace.IsNull()) {
        std::cerr << "failed to read Stiffener/TraceLine element in stiffener "
                     "with id "
                  << id << ", guid (" << guid << ")" << std::endl;
      } else {
        TDF_Label label = ctx->OCAFShapeTool()->AddShape(trace, false);
        TDataStd_Name::Set(label,
                           ("Stiffener " + id + " (" + guid + ")").c_str());
        ctx->OCAFColorTool()->SetColor(label, plateColor, XCAFDoc_ColorSurf);
        return trace;
      }
    }
