    ...
    if (nodeType == LDOM_Node::ELEMENT_NODE) {
      ...
      if ("Point3D" == OCXHelper::GetLocalTagName(pointN)) {
        if (pointCnt == 0) {
          ...
          std::cout << "found more that 3 points in  CircumCircle/Positions with curve id='" << id
                    << "'" << std::endl;
        }
        pointCnt++;
      } else {
        std::cout << "found element " << pointN.getTagName().GetString()
                  << " in  CircumCircle/Positions with curve id='" << id << "'" << std::endl;
      }
    }
    childN = childN.getNextSibling();
  }
...
TopoDS_Edge OCXCurveReader::ReadCircumArc3D(LDOM_Element &circleN) {
  ...
  if (intN.isNull()) {
    std::cout << "could not find CircumArc3D/IntermediatePoint in curve id='" << id << "'"
              << std::endl;
    return TopoDS_Edge();
  }
...
TopoDS_Shape OCXCurveReader::ReadNURBS3D(LDOM_Element &nurbs3DN) {
  ...
  if (numKnots != static_cast<int>(out.size())) {
    std::cerr << "failed to parse curve " << id << ", expected #" << numKnots
              << " knot values, but parsing [" << knotVectorS << "] resulted in #" << out.size()
              << " values"
              << std::endl;
    return TopoDS_Edge();
  }
  ...
  if (controlPointN.isNull()) {
    std::cout << "could not find NURBS3D/ControlPtList/ControlPoint in curve id='" << id << "'"
              << std::endl;
    return TopoDS_Edge();
  }
